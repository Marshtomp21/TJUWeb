...
function home() {
    ...
    //密码找回的中英文切换
    $("#pwd_url").unbind("click").bind("click", function () {
...
function changeLanguage() {
    let la = Boolean(getQueryString("locale")) ? getQueryString("locale") : (Boolean(getCookie("userLanguage")) ? getCookie("userLanguage") : defaultLanguage);
    if (la === "zh_CN") {
    ...
    $("#change_language").unbind("click").bind("click", function () {
...
function initPassWordEvent() {
    ...
    $("#qrcode_login").unbind().click(function () {
        ...
        let lqrcode = new loginQRCode("qrcode", 200, 200);

        lqrcode.generateLoginQRCode(function (result) {
            window.location.href = result.redirect_url;
        });
     
//        alert(document.getElementById("scanwords").innerHTML);
//        $("#robot-msg").text("可以扫码登录哟!");
        $("#robot-msg").text(document.getElementById("scanwords").innerHTML);
        $(".robot-mag-win").removeClass("big-small");
        $(".robot-mag-win").addClass("small-big-small");
